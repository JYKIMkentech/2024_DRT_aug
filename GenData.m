...
% Unimodal
mu_theta_uni = log(10);
sigma_theta_uni = 1.53 ; %1.536;
theta_min_uni = log(0.1) ; %mu_theta_uni - 3*sigma_theta_uni;
theta_max_uni = log(1000) ; %mu_theta_uni + 3*sigma_theta_uni;
...
theta_min_bi = log(0.1) ; %min([mu_theta1, mu_theta2]) - 3 * max([sigma_theta1, sigma_theta2]);
theta_max_bi = log(1000) ; %max([mu_theta1, mu_theta2]) + 3 * max([sigma_theta1, sigma_theta2]);
...
%% Plot Current Scenarios Only (Before Applying Resistance Distribution)
...
for idx = 1:length(scenarios_to_plot)
    ...
    yyaxis left
    plot(AS2_1per(s).t, AS2_1per(s).I, 'b-', 'LineWidth', 1.5);
    ylabel('Current (A)', 'FontSize', 12);
    hold on;
    
    yyaxis right
    plot(AS2_1per(s).t, AS2_1per(s).V, 'r-', 'LineWidth', 1.5);
    ylabel('Voltage (V)', 'FontSize', 12);
    
    xlabel('Time (s)', 'FontSize', 12);
    title(['Scenario ', num2str(AS2_1per(s).SN)], 'FontSize', 14);  % 시나리오 번호 사용
    legend('Current', 'Voltage', 'Location', 'best');
    hold off;
end
